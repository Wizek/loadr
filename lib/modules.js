...
.factory('protocolOf', function(protocolRxs) {
  return function protocolOf (name) {
    ...
    throw new Error('Unrecognized protocol for: ' + name)
  }
})
...
.factory('nameParser', function(logFactory, protocolOf, githubProtocolRx) {
  ...
  return function nameParser (name) {
    ...
    if (protocol == 'github') {
      ...
      var replaceStr = 'https://raw.github.com/$1/$2/'+ github.ref +'/$4'
...
.factory('resolve', function(nameParser, dependenciesOf, _, q, transc, logFactory) {
  ...
  console.log('init')
  return function resolve (depList, context) {
    ...
    function onEachBranch (branch) {
      return dependenciesOf(branch, context)
        .then(function(v) { return resolve(v, branch) })
...
.factory('getFileForRaw', function(_, q, fs, requestPapi, readdir, readFile, logFactory, nameParser, virtDepFile) {
...
.factory('getFileForWithErrorHandling', function(getFileForRaw) {
  return function(input) {
    return getFileForRaw.apply(this, arguments).then(null, function(err) {
      var r = JSON.stringify(err)
      return 'console.error("Failed to load: ' + input + '", ' + r + ')'
    })
  }
})

.factory('getFileFor', function(getFileForWithErrorHandling) {
  return getFileForWithErrorHandling
})

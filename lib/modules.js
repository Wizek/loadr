...
.value('localProtocolRx', /^\w[./\w]*$/)
.value('githubProtocolRx', /^github:\/\/([^/]+)\/([^/]+)(\/.*)?/)
.value('relativeProtocolRx', /^\.{1,2}\//)

.factory('protocolOf', function(httpProtocolRx, githubProtocolRx, relativeProtocolRx, localProtocolRx) {
  return function protocolOf (name) {
    ...
    if (name.match(relativeProtocolRx)) {
      return 'relative'
    }
    if (name.match(localProtocolRx)) {
      return 'local'
    }
...
.factory('getFileFor', function(_, q, fs, requestPapi, readdir, readFile, logFactory, nameParser, virtDepFile) {
  ...
  return function getFileFor (name) {
    ...
    if (parsedName.protocol == 'relative') {
      return q.when(virtDepFile(['asdasd']))
    }
...
.factory('cachedGetFileFor', function(memCache, getFileFor, logFactory) {
  ...
  // console.log('init')
  return function cachedGetFileFor (name) {
    var cached = memCache.get(name)
    if (cached) {
      console.log(name + ' pulled from cache')
      return cached
    } else {
      return memCache.set(name, getFileFor(name))
    }
...
.factory('dependenciesOf', function(cachedGetFileFor, sourceParser, logFactory) {
  ...
  return function(name) {
    ...
    return cachedGetFileFor(name).then(sourceParser)
  }
})

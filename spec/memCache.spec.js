...
describe('in-memory cache', function() {
  ...
  it('should keep track of time', function() {
    time = 1
    memCache.set('b', 1)
    expect(memCache._store.b).toBeSupersetOf({value:1,lastSet:1,lastGet:1})

    time = 2
    memCache.get('b')
    expect(memCache._store.b).toBeSupersetOf({value:1,lastSet:1,lastGet:2})

    time = 3
    memCache.set('b', 2)
    expect(memCache._store.b).toBeSupersetOf({value:2,lastSet:3,lastGet:3})
  })
